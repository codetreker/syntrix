...
func TestPullResponseDeadline(t *testing.T) {
	...
	rr := newPullRecorder()
	(pullHandler(service)).handlePull(rr, pullRequest(ctx, `{"collection":"users"}`))
...
func TestPullRejectsUnavailableResponseDeadline(t *testing.T) {
	for _, writer := range []http.ResponseWriter{
		...
		service := new(MockQueryService)
		(pullHandler(service)).handlePull(writer, pullRequest(context.Background(), `{"collection":"users"}`))
...
func TestHandlePullRejectsBeforeSource(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			service := new(MockQueryService)
			rr := newPullRecorder()
			(pullHandler(service)).handlePull(rr, pullRequest(context.Background(), tc.body))
...
func TestHandlePullFailureMapping(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(string(tc.code), func(t *testing.T) {
			...
			rr := newPullRecorder()
			(pullHandler(service)).handlePull(rr, pullRequest(context.Background(), `{"collection":"users"}`))
			assert.Equal(t, tc.status, rr.Code, rr.Body.String())
...
func TestHandlePullRejectsReassignedDatabaseAlias(t *testing.T) {
	...
	req = req.WithContext(databasecore.WithDatabase(req.Context(), &databasecore.Database{ID: "replacement-id", OwnerID: "pull-owner"}))
	rr := newPullRecorder()
	(pullHandler(service)).handlePull(rr, req)
...
func TestHandlePullLargeLegalPage(t *testing.T) {
	...
	rr := newPullRecorder()
	(pullHandler(service)).handlePull(rr, pullRequest(ctx, `{"collection":"users","limit":1000}`))
...
func pullHandler(service *MockQueryService) *Handler {
	return &Handler{engine: service, database: &fullMockDatabaseService{resolveFunc: func(ctx context.Context, _ string) (*databasecore.Database, error) {
		db, ok := databasecore.FromContext(ctx)
		if !ok {
			return nil, errors.New("missing test database")
		}
		return db, nil
	}}}
}

func TestHandlePullStrictQuerySource(t *testing.T) {
	for _, body := range []string{
		`{"collection":"users","source":null}`,
		`{"collection":"users","source":{"version":1}}`,
		`{"collection":"users","source":{"version":1.0,"filters":[]}}`,
		`{"collection":"users","source":{"version":1,"filters":[],"unknown":true}}`,
		`{"collection":"users","source":{"version":1,"filters":[]},"unknown":true}`,
		`{"collection":"users","source":{"version":1,"filters":[]},"source":{"version":1,"filters":[]}}`,
		`{"collection":"users","source":{"version":1,"filters":[{"field":"x","op":"==","value":{"type":"int64","value":"1","value":"2"}}]}}`,
		`{"collection":"users","source":{"version":1,"filters":[{"field":"x","op":"==","value":{"type":"string","value":"\ud800"}}]}}`,
		`{"collection":"users","source":{"version":1,"filters":[]},"requestId":"unexpected"}`,
		`{"collection":"users","source":{"version":1,"filters":[],"limit":5}}`,
		`{"collection":"users","source":{"version":1,"filters":[],"limit":5},"requestId":"window","limit":0}`,
		`{"collection":"users","source":{"version":1,"filters":[],"limit":5},"requestId":"window","checkpoint":null}`,
		`{"collection":"users","source":{"version":1,"filters":[]},"limit":1.5}`,
		`{"collection":"users","source":{"version":1,"filters":[]},"limit":1001}`,
		`{"collection":"users","source":{"version":1,"filters":[]},"limit":null}`,
	} {
		t.Run(body, func(t *testing.T) {
			service := new(MockQueryService)
			rr := newPullRecorder()
			pullHandler(service).handlePull(rr, pullRequest(context.Background(), body))
			require.Equal(t, 400, rr.Code, rr.Body.String())
			require.Contains(t, rr.Body.String(), `"code":"INVALID_REPLICATION_SOURCE"`)
			service.AssertNotCalled(t, "Pull", mock.Anything, mock.Anything, mock.Anything)
		})
	}
...
func TestHandlePullQuerySourceTypedAndEmptyPage(t *testing.T) {
	source := &storagetypes.ReplicationSource{Version: 1, Filters: model.Filters{{Field: "score", Op: model.OpEq, Value: int64(math.MaxInt64)}}}
	service := new(MockQueryService)
	service.On("Pull", mock.Anything, "friendly-name", storage.ReplicationPullRequest{Collection: "users", DatabaseIdentity: "canonical-id", Limit: 100, Source: source}).Return(&storage.ReplicationPullResponse{
		ProtocolVersion: 1, Mode: "events", DatabaseIdentity: "canonical-id", SourceHash: strings.Repeat("a", 64), GenerationID: "generation", Phase: "scan", Checkpoint: "next", Events: []storagetypes.ReplicationEvent{},
	}, nil).Once()
	rr := newPullRecorder()
	pullHandler(service).handlePull(rr, pullRequest(context.Background(), `{"collection":"users","source":{"version":1,"filters":[{"field":"score","op":"==","value":{"type":"int64","value":"9223372036854775807"}}]}}`))
	require.Equal(t, 200, rr.Code, rr.Body.String())
	var response map[string]json.RawMessage
	require.NoError(t, json.Unmarshal(rr.Body.Bytes(), &response))
	assert.NotContains(t, response, "documents")
	assert.JSONEq(t, "[]", string(response["events"]))
