...
func pullCheckpoint(database, collection string) string {
	return base64.RawURLEncoding.EncodeToString([]byte(fmt.Sprintf(`{"version":2,"database":%q,"databaseIdentity":"canonical-id","collection":%q,"phase":"changes","position":"native-position"}`, database, collection)))
}
	...
	for _, checkpoint := range []string{"", `,"checkpoint":null`, `,"checkpoint":""`, `,"checkpoint":"` + pullCheckpoint("friendly-name", "users") + `"`} {
		t.Run(checkpoint, func(t *testing.T) {
			...
			service.On("Pull", mock.Anything, "friendly-name", mock.MatchedBy(func(req storage.ReplicationPullRequest) bool {
				...
				require.True(t, ok)
				require.WithinDuration(t, time.Now().Add(storagetypes.ReplicationHardTimeout), deadline, time.Second)
...
func TestHandlePullFailureMapping(t *testing.T) {
	for _, tc := range []struct {
		code   storagetypes.ReplicationErrorCode
		status int
	}{
		{storagetypes.ReplicationInvalidCursor, 400}, {storagetypes.ReplicationScopeMismatch, 400},
		{storagetypes.ReplicationSourceMismatch, 409}, {storagetypes.ReplicationHistoryUnavailable, 409},
		{storagetypes.ReplicationIdentityUnavailable, 409}, {storagetypes.ReplicationUnsupported, 501},
		{storagetypes.ReplicationUnavailable, 503}, {storagetypes.ReplicationPermissionDenied, 403},
		{storagetypes.ReplicationBudgetExceeded, 422}, {storagetypes.ReplicationInvalidState, 500},
	} {
		t.Run(string(tc.code), func(t *testing.T) {
			service := new(MockQueryService)
			service.On("Pull", mock.Anything, "friendly-name", mock.Anything).Return(nil, &storagetypes.ReplicationError{Code: tc.code, Cause: errors.New("private-cursor-and-payload")}).Once()
			rr := httptest.NewRecorder()
