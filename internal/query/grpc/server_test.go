...
func TestServerPullRejectsLegacyNumericProtobufBeforeService(t *testing.T) {
	...
	require.Equal(t, string(types.WatchInvalidCheckpoint), info.Reason)
...
func TestServerPullDiscardsFailedResponses(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "source history unavailable", err: &types.WatchError{Code: types.WatchHistoryUnavailable, Cause: fmt.Errorf("private native position")}, code: codes.FailedPrecondition},
