...
an equality condition for update/delete on live targets. Create ignores a valid
version, rejects live targets with `already_exists`, and allows missing or
tombstoned targets. `current` is absent only for a missing conflict target;
`change_index` identifies the original request item. These
