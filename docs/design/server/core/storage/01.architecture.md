...
5. **Factory / Stack (The Assembler)**
   ...
   - **Readiness**: Required initialization must succeed before the Factory returns a usable stack. PostgreSQL connection checks and DDL share the caller's context; [schema initialization](06.user-store-postgres.md#7-schema-initialization) coordinates concurrent instances with a database-wide transaction lock.
