...
源端临时 5xx、网络失败和限流走读取退避；持久化失败不按网络错误重试。
[私有 WS/HTTP 源运输](2026-09-21-sdk-replica-websocket.md)已复用有界轮次与 hint，
并在最后本地分块、manifest/pin 完成后取得整页回执；运输切换不复制本地应用器。
...
- 公开 facade 已组合下行复制、成员、pin、查询和上行/恢复；周期核对沿当前 WS/HTTP 运输执行，不依赖变化提示完整到达。
