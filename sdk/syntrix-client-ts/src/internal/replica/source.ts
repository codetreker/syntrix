...
import type { ReplicaSourceAdapter, SourceDocument, SourceEvent, SourceEventsPage, SourceReadContext, SourceWindow } from './source-types.js';
...
const parseEnvelope = (text: unknown): unknown => {
  if (typeof text !== 'string') return invalid('Source transport must return JSON text');
  // Reject before UTF-8 allocation and JSON/typed decoding. Axios also enforces
  // maxContentLength on adapters that support streaming transport limits.
...
): SourceEventsPage | SourceWindow => decodeEnvelope(parseEnvelope(text), definition, context);
...
export const createReplicaHttpSource = (options: {
  ...
  if (!validText(options.database) || !validText(definition.collection) ||
      (definition.limit !== null && (!Number.isInteger(definition.limit) || definition.limit < 1 || definition.limit > 1000))) {
    return invalid('Invalid source database, collection, or window limit');
  }
  ...
  return {
    ...
    async read(context) {
      ...
      const source = { version: 1, filters: definition.filters, orderBy: definition.orderBy,
        ...(mode === 'replace' ? { limit: definition.limit } : {}) };
      ...
      const config: AxiosRequestConfig & { _syntrixAuthSessionVersion: number } = {
        ...
        transformResponse: [(data: unknown, _headers: unknown, status?: number) => {
          if (status !== undefined && (status < 200 || status >= 300)) {
            ...
            try { return parseEnvelope(data); } catch (error) {
